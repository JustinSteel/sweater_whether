...
RSpec.describe "Munchies API" do
  describe "happy path" do
    it "can return the destination city and state name as a string", :vcr do
      get "/api/v1/munchies?destination=pueblo,co&food=mexican"
    ...
    it "returns the restaurant that matches the food search", :vcr do
      ...
      response_data = JSON.parse(response.body, symbolize_names: true)
        ...
        expect(response_data[:data][:attributes][:restaurant][:name]).to eq("Tacos Navarro")
        ...
        expect(response_data[:data][:attributes][:restaurant][:address]).to eq("223 E Northern Ave, Pueblo, CO 81006")
        ...
        expect(response_data[:data][:attributes][:restaurant][:rating]).to eq(4.5)
        ...
        expect(response_data[:data][:attributes][:restaurant][:reviews]).to eq(138)
    end
  end
end